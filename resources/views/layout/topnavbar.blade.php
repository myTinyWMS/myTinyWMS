            ...
            <h3 class="mr-6 h-full pt-5 {{ active_class(if_uri_pattern(['order*'])) }}">
                <a href="{{ url('/order') }}">
                    ...
                    @if($globalPageService->getUnreadMessageCount())
                        <div class="ml-2 inline-block bg-blue-700 text-white rounded-full text-center w-6 h-6 text-sm leading-none pt-1" title="{{ $globalPageService->getUnreadMessageCount() }} ungelesene {{ trans_choice('plural.message', $globalPageService->getUnreadMessageCount()) }}">{{ $globalPageService->getUnreadMessageCount() }}</div>
                    @endif
                </a>
            </h3>
