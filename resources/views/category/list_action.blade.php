...
<form action="{{ route('category.destroy', $id) }}" class="list-form" method="POST">
    ...
    @if ($model->articles->count() > 0)
        <button class="table-action btn-link" title="Dieser Kategorie sind noch Artikel zugeordnet. Sie kann nicht gelöscht werden!" disabled="disabled">Löschen</button>
    @else
