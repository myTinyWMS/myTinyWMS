...
<div class="inline-count-info pull-right">
    <a class="count-info relative" href="{{ route('order.show', $order) }}" title="{{ $order->messages->where('read', false)->count() }} @lang('ungelesen') {{ trans_choice('plural.message', $order->messages->where('read', false)->count()) }}">
