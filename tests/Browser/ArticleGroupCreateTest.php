    ...
    public function test_create_new_article_group() {
        $this->browse(function (Browser $browser) {
            ...
            $browser
                ...
                ->type('.dataTables_filter input', $article1->internal_article_number)
                ->waitUntilMissing('#dataTableBuilder_processing')
                ->pause(1000)
                ->assertSee($article1->internal_article_number)
                ->press('auswählen')
                ->waitFor('#quantity_0')
                ->pause(1000)
                ->assertSee($article1->internal_article_number)
                ...
                ->click('#add-article')
                ->waitUntilMissing('#dataTableBuilder_processing')
                ->type('.dataTables_filter input', $article2->internal_article_number)
                ->waitUntilMissing('#dataTableBuilder_processing')
                ->pause(1000)
                ->assertSee($article2->internal_article_number)
                ->press('auswählen')
                ->waitFor('#quantity_1')
                ->pause(1000)
                ->assertSee($article2->internal_article_number)
